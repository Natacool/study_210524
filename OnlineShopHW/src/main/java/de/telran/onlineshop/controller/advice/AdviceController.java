...
public class AdviceController {
    ...
    @ExceptionHandler(MethodArgumentNotValidException.class) //WebExchangeBindException.class ----> (WEB view, NOT REST view)
    ...
    private Map<String, List<String>> getErrorsMap(List<String> errors) {
        ...
        errorResponse.put("errors", errors); //.sort()
        return errorResponse;
    }

    // альтернативная обработка ошибочной ситуации Exception
    @ExceptionHandler(Exception.class) //Throwable.class //RuntimeException.class exception
    public ResponseEntity handleException(Exception exception) {
        return ResponseEntity
                .status(HttpStatus.I_AM_A_TEAPOT)
                .body("AdviceController: " + "Извините, что-то пошло не так. Попробуйте позже!");
    }
}
